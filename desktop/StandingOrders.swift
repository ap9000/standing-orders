...
final class DesktopApp: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if args.contains("--state") { label += ".preview" }
        do {
            ...
            makeWindow()
            makeMenu()
            settings = try inspect()
            login = try? readLogin()
            if login == nil && settings!.importable {
                let imported = try JSONDecoder().decode(Login.self, from: command("import-login"))
                try saveLogin(imported)
                login = imported
                // Delete the legacy file only after a successful Keychain readback.
                if try readLogin().password == imported.password { _ = try command("remove-legacy", input: imported) }
            }
            if login == nil { signIn() }
            if settings!.repos.isEmpty { chooseRepository() }
            else { startService() }
    ...
    func makeWindow() {
        ...
        bar.spacing = 10
        for (title, selector) in [("Overview", #selector(openOverview)), ("Sessions", #selector(openControl)), ("Refresh", #selector(refresh)), ("Sign in", #selector(signIn))] {
    ...
    func process(_ executable: String, _ args: [String], input: Data? = nil) throws -> Data {
        let task = Process(); task.executableURL = URL(fileURLWithPath: executable); task.arguments = args
        var environment = ProcessInfo.processInfo.environment
        environment["PATH"] = [URL(fileURLWithPath: node).deletingLastPathComponent().path, FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".local/bin").path, "/opt/homebrew/bin", "/usr/local/bin", "/usr/bin", "/bin", "/usr/sbin", "/sbin"].joined(separator: ":")
        ...
        if task.terminationStatus != 0 {
            throw NSError(domain: "StandingOrders", code: Int(task.terminationStatus), userInfo: [NSLocalizedDescriptionKey: String(data: errorData, encoding: .utf8) ?? "The local command failed."])
        }
        return result
    }
    func command(_ name: String, extra: [String] = [], input: Login? = nil) throws -> Data {
        try process(node, [helper, name] + extra + ["--state", stateDir.path], input: input.map { try! JSONEncoder().encode($0) })
    }
    func inspect() throws -> Settings { try JSONDecoder().decode(Settings.self, from: command("inspect")) }
    ...
    @objc func signIn() {
        do {
            ...
            alert.informativeText = "Your sign-in is saved in macOS Keychain. Approval buttons still ask you to review the exact action."
            ...
            _ = try command("pair", input: value)
            if !first { try saveLogin(value) }
    ...
    @objc func chooseRepository() {
        ...
        do {
            _ = try command("add-repos", extra: panel.urls.map { $0.path }); settings = try inspect()
            startService()
        } catch { showError(error.localizedDescription) }
    }
    ...
    @objc func startService() {
        do {
            ...
            if (try? process("/bin/launchctl", ["print", "gui/\(getuid())/\(label)"])) == nil { _ = try process("/bin/launchctl", ["bootstrap", "gui/\(getuid())", path.path]) }
            else { _ = try? process("/bin/launchctl", ["kickstart", "gui/\(getuid())/\(label)"]) }
    ...
    func connect(attempts: Int) {
        ...
        URLSession.shared.dataTask(with: base.appendingPathComponent("desktop/health")) { data, _, _ in
            let identity = data.flatMap { try? JSONSerialization.jsonObject(with: $0) as? [String: String] }?["identity"]
    ...
    @objc func about() {
        let alert = NSAlert(); alert.messageText = "Standing Orders"; alert.informativeText = "A local console for approved coding work. The background service runs independently of this window. Use Sessions to start or stop work."; alert.runModal()
    }
    @objc func stopService() {
        let alert = NSAlert(); alert.messageText = "Stop the background service?"
        alert.informativeText = "This stops the local console and its workers. Unfinished tasks are paused with their work preserved. Closing the window alone keeps work running."
        alert.addButton(withTitle: "Stop service"); alert.addButton(withTitle: "Keep running")
        if alert.runModal() != .alertFirstButtonReturn { return }
        do { _ = try process("/bin/launchctl", ["bootout", "gui/\(getuid())/\(label)"]); status.stringValue = "Background service stopped. File → Start background service to return." }
        catch { showError(error.localizedDescription) }
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        // Only password confirmation inputs on our pinned loopback console.
        // Values are arguments, never executable JavaScript string fragments.
        if let login {
            webView.callAsyncJavaScript("for (const field of document.querySelectorAll('input[type=password][name=token][autocomplete=current-password]')) { field.value = secret; }", arguments: ["secret": login.password], in: nil, in: .page, completionHandler: nil)
        }
    }

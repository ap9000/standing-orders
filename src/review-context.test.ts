...
describe("inherited review context (v51)", () => {
  ...
  const seed = async (options: {
    ...
    // `revision`.
    const brief = { schema: 1, sourceTask: "feat", sourceRun, head: source, diffArtifactSha: store.getArtifact(sourceDiffArtifact)!.sha256, comments: [] };
    ...
    const sealed = store.sealRevision(
      {
        task: { id: "feat-rev", title: "Revise feat", repo, goal: "cap retries — apply the annotations", touches: [], acceptance: options.revisionRubric ?? RUBRIC, ...(options.strict === true ? { qualityMode: "strict" as const } : {}) },
        artifact: { run: sourceRun, kind: "revision-brief", key, bytesOriginal: briefBytes.length, bytesStored: briefBytes.length, truncated: false, sha256: sha256(briefBytes), capture: "machine-authored revision brief (exit 0)" },
        revisionOf: "feat",
        commentIds: null,
        sourceRun,
      },
    ...
    if (options.strict === true) {
      ...
      propose(store, { taskId: revisionTaskId, goal: drafted.goal, acceptance: drafted.acceptance, qualityMode: "strict", now: T0 });
    }

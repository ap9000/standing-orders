...
export function isWorkspace(value: unknown): value is BrowserWorkspace {
  ...
  if (!Array.isArray(value.navigation) || !value.navigation.every(item => record(item) && typeof item.label === "string" && typeof item.href === "string" && typeof item.active === "boolean")) return false;

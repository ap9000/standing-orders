...
export function ViewHost({ view, csrf }: { view: BrowserView; csrf: string }) {
  return <div data-view={view.kind} className="w-full">
    {view.kind === "tasks" ? <TasksView view={view} /> : view.kind === "settings" ? <SettingsView view={view} csrf={csrf} /> : view.kind === "task" ? <TaskView view={view} /> : view.kind === "projects" ? <ProjectsView view={view} csrf={csrf} /> : null}
  </div>;
}

...
export const COMMAND_GUIDE: readonly CommandRow[] = [
  ...
  { invocation: "task steer", synopsis: "one note (≤500 chars) read before the task's next attempt — guidance inside the approved scope, never an interruption", audience: "agent", agentMayInvoke: true, mutation: "keyed",
    positionals: [{ name: "id", required: true, meaning: "the task" }],
    flags: [jsonFlag, dbFlag, keyFlag, { name: "note", takesValue: true, meaning: "the guidance" }],
    notableReasons: ["unknown-task", "contest-open"] },
  ...
  { invocation: "task plan", synopsis: "ask for a plan document (a planning session drafts; a person approves)", audience: "agent", agentMayInvoke: true, mutation: "keyed",
    positionals: [{ name: "id", required: true, meaning: "the task" }],
    flags: [jsonFlag, dbFlag, keyFlag], notableReasons: ["unknown-task"] },

...
/** v62 (the conversation queue) rides above this foundation; a v60 wind-back drops it too. */
const laterTables = ["telegram_conversation", "telegram_proposal_action"];
...
describe("v61 Telegram delivery foundation", () => {
  ...
  test.each([60, -60])("v%s preserves legacy receipts and unknown provenance, then reopens idempotently", version => {
    ...
    expect(store.handle.prepare("SELECT version FROM schema_version").get()?.["version"]).toBe(62);

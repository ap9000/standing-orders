...
describe('coding workspace HTML', () => {
  ...
  test('has truthful start terms, one supported provider, normal form fields and an unavailable state', () => {
    ...
    expect(output).toContain('Codex uses this installation’s login and tools.');
    expect(output).toContain('It can edit a separate copy of this project, run commands, and read files outside the project.');
    expect(output).toContain('Extra access requires your approval.');

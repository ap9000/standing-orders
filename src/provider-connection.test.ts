...
test("identity output is normalized, bounded, and never turns an unknown status into Connected", () => {
  ...
  expect(signInFacts("codex", { ...OK, stderr: "Logged in using an API key" }).state).toBe("unverified");
  for (const authMethod of ["api_key", "bedrock", "vertex", "unknown"]) {
    expect(signInFacts("claude", { ...OK, stdout: JSON.stringify({ loggedIn: true, authMethod }) }).state).toBe("unverified");
  }
});
...
test("setup, connection settings, and settings agree on real probe facts; refresh detects sign-out without exposing raw output", async () => {
  const root = mkdtempSync(join(tmpdir(), "so-connection-http-")); const repo = join(root, "repo"); mkdirSync(repo); execFileSync("git", ["init", "-q", repo]);
  ...
  const server = createDecisionServer({ store, repo, repos: [repo], evidenceRoot: root, telegramTokenFile: join(root, "absent-token"), connectionHome: root, connectionProbe: async () => { probes++; return result; } });
  ...
  try {
    ...
    expect(parse(settings).querySelector('form:has(input[name="provider"][value="claude"]) .provider-connection')?.textContent).toContain("Connected");
    result = { ...OK, code: 1, stdout: '{"loggedIn":false}' };
    ...
    expect(parse(unknown).querySelector('form:has(input[name="provider"][value="claude"]) .provider-connection')?.textContent).toContain("Not verified");

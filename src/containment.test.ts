...
describe("the capability, established by doing", () => {
  ...
  test("linux: a delegated cgroup is proven by creating and removing a child; cgroup.kill is reported truthfully", () => {
    writeFileSync(join(dir, "cgroup.controllers"), "cpu memory\n");
    mkdirSync(join(dir, "own"));
    const without = probeContainmentCapability({ platform: "linux", cgroupMount: dir, selfCgroup: "0::/own\n" });
    expect(without).toMatchObject({ available: true, backend: "cgroup2", cgroupRoot: join(dir, "own"), cgroupKill: false });
    expect(without.detail).toContain("pid by pid");
    ...
    expect(withKill).toMatchObject({ available: true, cgroupKill: true });
      ...
      return {
        ...
        attach(child) {
          return new Promise<AttachOutcome>(resolve => {
            const channel = child.stdio[3] as import("node:stream").Readable;
            let seen = "";
            channel.on("data", chunk => { seen += String(chunk); if (seen.startsWith("attached")) resolve({ ok: true }); });

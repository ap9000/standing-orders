...
export type RunOptions = ProcessTreeObserver & {
  ...
  /** Fires right after onSpawn when the spawn has a native OS object —
   * the exact-run custody record names it beside the pid. */
  onContainer?: (info: { backend: ContainmentBackendId; id: string }) => void;
...
function spawnContained(
  ...
  label: string,
  bag: { beforeSpawn?: (() => boolean) | undefined; onSpawn?: ((pid: number) => void) | undefined; onContainer?: ((info: { backend: ContainmentBackendId; id: string }) => void) | undefined },
  contain = true,
): ContainedSpawn {
  ...
  const launch = container === null ? null : container.launch(file, args);
  ...
  if (container !== null) containers.set(child, container);
  try {
    if (child.pid !== undefined) {
      bag.onSpawn?.(child.pid);
      if (container !== null) bag.onContainer?.({ backend: container.backend, id: container.id });
    }
  } catch (error) {
 ...
 */
async function settleContainer(container: Container, bag: { onContainerEmpty?: (() => void) | undefined; onUnknown?: (() => void) | undefined }, timeoutMs = 5_000): Promise<boolean> {
  let empty = container.populated() === false;
  if (!empty) empty = await container.kill(timeoutMs);
  try {
...
function spawnStream(
  ...
  return spawnContained(
    ...
    { beforeSpawn: options.beforeSpawn, onSpawn: options.onSpawn, onContainer: options.onContainer },
  ...
  return new Promise(resolve => {
    ...
    try {
      const spawned = spawnContained(
        ...
        bag.owner ?? "buffered",
        { beforeSpawn: bag.beforeSpawn, onSpawn: bag.onSpawn, onContainer: bag.onContainer },
      );
      child = spawned.child;
    ...
    const finish = (code: number | null): void => {
      ...
      void settlement.then(empty => {
        resolve({
          result: {
            code: notFound ? NOT_FOUND_CODE : overflowed ? OVERFLOW_CODE : timedOut ? TIMEOUT_CODE : attachFailure !== null ? CONTAINMENT_REFUSED_CODE : (code ?? 1),
            stdout,
            stderr: attachFailure !== null && stderr === "" ? attachFailure : stderr,
  ...
  return new Promise(resolve => {
    ...
    const finish = (code: number | null): void => {
      ...
      void settlement.then(empty => resolve({
        code: notFound ? NOT_FOUND_CODE : timedOut ? TIMEOUT_CODE : inputFailed ? 1 : attachFailure !== null ? CONTAINMENT_REFUSED_CODE : (code ?? 1),
        stdout: lines.join("\n"),
        stderr: attachFailure !== null && stderr === "" ? attachFailure : stderr,
        timedOut,
        notFound,
...
export function runGeminiStreamJsonl(
  ...
  return new Promise(resolve => {
    ...
    const finish = (code: number | null): void => {
      ...
      const settlement = container === null ? Promise.resolve<boolean | null>(null) : settleContainer(container, options);
      void settlement.then(empty => resolve({
        code: notFound ? NOT_FOUND_CODE : timedOut ? TIMEOUT_CODE : attachFailure !== null ? CONTAINMENT_REFUSED_CODE : (code ?? 1),
        stdout: lines.join("\n"),
        stderr: attachFailure !== null && stderr === "" ? attachFailure : stderr,
        timedOut,
        notFound,

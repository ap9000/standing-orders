...
export type RestartBaseline = {
  ...
  runtime: { nodeVersion: string; execPath: string; platform: NodeJS.Platform; packageVersion: string | null };
...
export async function recordRestartBaseline(inputs: BaselineInputs): Promise<RestartBaseline> {
  ...
  return {
    ...
    runtime: { nodeVersion: process.version, execPath: process.execPath, platform: process.platform, packageVersion: inputs.packageVersion ?? readPackageVersion() },
...
export async function verifyRestartRecovery(inputs: VerifyInputs): Promise<RestartVerification> {
  ...
  const current = await recordRestartBaseline({ ...inputs, boot: identity });
  ...
  if (baseline.service !== null || current.service !== null) {
    ...
    if (service === null) checks.push({ name: "service", ok: false, detail: "no service definition was given to verify against" });
      ...
      checks.push({ name: "service", ok: service.state === "running" && service.problems.length === 0, detail: `${service.label}: ${service.detail}${service.stale ? " (installed definition is stale: reinstall to load the current one)" : ""}${problems}` });
    }
  }
  ...
  const fresh = liveRunners.filter(one => one.alive && (baselineBeats.get(one.name) === undefined || one.heartbeatAt > (baselineBeats.get(one.name) ?? "")));
  checks.push({
    name: "heartbeat",
    ok: fresh.length > 0,
  ...
  if (inputs.recover !== false) store.settleQuiescentStops(inputs.now ?? new Date());

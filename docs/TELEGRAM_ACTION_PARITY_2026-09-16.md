...
checks every row below against that constant, so the code and this document
...
- **handoff** — the phone names the existing authenticated control and the
  task; it does nothing itself and sends no link.
...
| `show_control` | handoff | The card names the control and the task; the operator opens it on the computer. No link is sent. | control card test: no button, no link, a forged tap changes nothing | none |
| `propose_task` | direct | Card with Confirm/Dismiss through confirmMateProposal (filed as a mate proposal, via telegram). Scope approval stays on the computer. | first test: filed task, outcome `via: telegram`, replay does nothing twice | none |
| `propose_scope` | direct | Confirm rewrites the scope through the shared door; the password approval that follows is a handoff. | scope card test | none |
...
| `propose_agents` | direct | Confirm through the shared route-edit door; renewed approval stays on the computer. | agents card test (route override written) | none |
...
| `propose_task_action` | direct | stop, retry, plan, wait_for and stop_waiting confirm through the shared door (a stop is audited via telegram); resume confirms only the request and hands off to the password step. | task-action card test: retry, plan, wait_for, stop_waiting, stale refusal, stop (`run_stop.requested_via = telegram`); resume card copy | resume completes on the computer. |
...
| `propose_review` | direct | note saves feedback; revise creates the same-family revision through the shared result service, honouring automatic approval settings. | reply-to-result test (revise), review note card test | none |
| `propose_cancel` | handoff | The door refuses cancel from any card; the phone says to arm it on the task itself. | cancel card test: no button, no link, a forged tap changes nothing | No phone path to cancel by design. |
...
- Ordinary text is persisted before the poll cursor moves. The engine's
  request receipt is derived from bot, binding and update, so a replay or a
  restart recovers the original turn instead of dispatching again.

...
The worker does not execute a separate model review. Historical reviews and signed settings remain recorded; they do not keep finished work waiting for another reviewer. Revisions require explicit feedback and the existing execution approval. The lead receives a durable inbox batch and acknowledges delivery after handling it; the database saves its position. A supported host watcher or scheduled follow-up wakes the lead. A fresh database brief supplies catch-up context. See [Agent updates](AGENT_UPDATES.md).

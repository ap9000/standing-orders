...
Start the task branch at the verified main base above. The authenticated steering note pins the final prepared evidence commit. Read this document from that commit with git show, then apply the two commits from the verified base through that prepared commit, preserving the whole-task base. Do not start the task at the prepared implementation or move its original base later.

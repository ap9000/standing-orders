...
Three bounded, disposable checks accompany the OS containment and login
...
npm run certify:restart -- baseline --db ~/.config/standing-orders/orders.db --label com.standing-orders.watch.<slug>
...
npm run certify:launchd                               # macOS only: a DISPOSABLE LaunchAgent, relaunch after exit 0 and SIGKILL, ≥ 60 s
```
...
controller's own rules, and task completion. Its output lists the exact
...
disposable install is acceptable. Neither has been run against this
machine's live installation by the task builder — the supervising operator
owns that.

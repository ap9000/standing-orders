...
const usage = "npm run certify:restart -- baseline|verify --db <file> [--label <label>] [--baseline <file>] [--expect reboot|login|either] [--output <file>] [--runner <name>] [--task <id>] [--no-recover]";
const [mode, ...rest] = process.argv.slice(2);
if (mode === undefined || mode === "--help" || (mode !== "baseline" && mode !== "verify")) { console.log(usage); process.exit(mode === "--help" ? 0 : 2); }
const options = { db: undefined, label: undefined, baseline: undefined, expect: "either", output: undefined, recover: true, runner: undefined, tasks: [] };
for (let index = 0; index < rest.length; index++) {
  ...
  else if (arg === "--task") options.tasks.push(value);
...
try {
  ...
  if (mode === "baseline") {
    const baseline = await recordRestartBaseline({ store, ...(options.runner ? { runnerName: options.runner } : {}), completionTasks: options.tasks, ...(service === undefined ? {} : { service }) });

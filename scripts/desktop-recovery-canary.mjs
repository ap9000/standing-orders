...
let runtimeRoot=join(source,'dist'),node=process.execPath,associatedBundleId;
if(app){const resources=realpathSync(join(resolve(app),'Contents/Resources'));const runtime=JSON.parse(readFileSync(join(resources,'runtime.json'),'utf8'));node=resolve(resources,runtime.node);runtimeRoot=join(resources,'dist');associatedBundleId=runtime.bundleId??'com.standing-orders.desktop';}
...
const {currentDesktopAccess}=await import(pathToFileURL(join(runtimeRoot,'desktop-access.js')));
...
writeFileSync(plist,launchdPlist({label,command:[node,join(runtimeRoot,'desktop-host.js'),'serve','--state',state],workingDirectory:state,pathEnv:env.PATH,environment:env,logPath:join(root,'service.log'),...(associatedBundleId?{associatedBundleId}:{})}),{mode:0o600});

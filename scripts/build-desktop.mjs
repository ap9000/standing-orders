#!/usr/bin/env node
import { cpSync, mkdirSync, mkdtempSync, readFileSync, realpathSync, renameSync, writeFileSync, rmSync, existsSync } from "node:fs";
...
if (existsSync(destination)) {
  if (!existsSync(join(destination, "Contents", "Resources", "standing-orders-bundle"))) throw new Error("Output exists and is not a Standing Orders build.");
}
...
execFileSync("/usr/bin/swiftc", ["-O", "-parse-as-library", "-swift-version", "5", "-framework", "AppKit", "-framework", "WebKit", "-framework", "Security", join(root, "desktop", "StandingOrders.swift"), "-o", join(macos, "StandingOrders")], { stdio: "inherit" });
